...
//        Util.debug("Parsing complete in " + (System.currentTimeMillis() - start) + " millis.");
    ...
    private static class SVGHandler extends DefaultHandler {
        ...
        private boolean doFill(Attributes atts, HashMap<String, Shader> gradients) {
            ...
            String fillString = getStringAttr("fill", atts);
            if (fillString != null && fillString.startsWith("url(#")) {
                ...
                Integer color = getHexAttr("fill", atts);
                if (color != null) {
                    doColor(atts, color);
                    paint.setStyle(Paint.Style.FILL);
                    return true;
                } else if (getStringAttr("fill", atts) == null && getStringAttr("stroke", atts) == null) {
        ...
        private boolean doStroke(Attributes atts) {
            if (whiteMode) {
            ...
            Integer color = getHexAttr("stroke", atts);
            if (color != null) {
                doColor(atts, color);
                // Check for other stroke attributes
                Float width = getFloatAttr("stroke-width", atts);
                // Set defaults

                if (width != null) {
                    paint.setStrokeWidth(width);
                }
                String linecap = getStringAttr("stroke-linecap", atts);
                ...
                String linejoin = getStringAttr("stroke-linejoin", atts);
        ...
        private Gradient doGradient(boolean isLinear, Attributes atts) {
            ...
            if (isLinear) {
                gradient.x1 = getFloatAttr("x1", atts);
                gradient.x2 = getFloatAttr("x2", atts);
                gradient.y1 = getFloatAttr("y1", atts);
                gradient.y2 = getFloatAttr("y2", atts);
            } else {
                gradient.x = getFloatAttr("cx", atts);
                gradient.y = getFloatAttr("cy", atts);
                gradient.radius = getFloatAttr("r", atts);
            }
        ...
        private void doColor(Attributes atts, Integer color) {
